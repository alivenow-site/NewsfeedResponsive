...
class ResponsiveGame extends SignalHub {
    ...
    init(defaultData) {
        this.orientation = defaultData.orientation;
        this.scaleOnHeight=(defaultData.scaleOnHeight||false)
        this.defaultVal = defaultData;
        this.initSignal();
        ...
        var c_width = ((this.isMobile) ? (this.orientation == "portrait" ? Math.min(window.innerWidth, window.innerHeight) : Math.max(window.innerWidth, window.innerHeight)) : (this.orientation == "portrait") ? defaultData.defaultDimension.width : window.innerWidth) *window.devicePixelRatio;
        var c_height = ((this.isMobile) ? (this.orientation == "portrait" ? Math.max(window.innerWidth, window.innerHeight) : Math.min(window.innerWidth, window.innerHeight)) : (this.orientation == "portrait") ? defaultData.defaultDimension.height : window.innerHeight) *window.devicePixelRatio;
        ...
        Object.keys(this.dimensionArr).forEach(function (_item, key) {
            if (_item == null || (_item.name == key && _item.alive)) {
                ...
                if (ele._isBG) {
                    ...
                    if (ele._item.type == 4) {
                        if(this.scaleOnHeight){
                            ele._item.fontSize = ele._item.initFontSize * _height / this.gameResizeFactor.h;
                        }else{
               ...
               /*          ele._item.setScale(ele._w * _width / 1000,ele._w * _width / 1000) */
                            if(this.scaleOnHeight){
                                ele._item.scale.setTo(ele._h * _height / 1000);
                            }else{
    ...
    updateItemScale(_scene, _key, _newResizeFactor, saveOnComplete) {
        Object.keys(this.dimensionArr).forEach(function (key) {
            ...
            if (_key == key) {
                ...
                if(this.scaleOnHeight){
                    this.dimensionArr[_key]._item.scale.setTo(this.dimensionArr[_key]._h * _scene.game.canvas.height / 1000, Math.abs(this.dimensionArr[_key]._h) * _scene.game.canvas.height / 1000);
                }else{
